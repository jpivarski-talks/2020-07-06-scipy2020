...
\begin{frame}{How particle physics analyses generally work}
\large
\begin{columns}
...
\vspace{-11 cm}
\textcolor{white}{\hspace{0.25 cm} But tracks come unlabeled: it's a large combinatorics problem per event}

\vspace{11 cm}
\end{columns}
\end{frame}

\begin{frame}{Automatically identifying a particle decay: ${K_s}^0 \to \pi^+\pi^-$}
\large
\vspace{0.5 cm}
\begin{enumerate}
...
\begin{frame}{We've {\it always} needed variable-length lists and data structures}
\large
\vspace{0.5 cm}

\textcolor{darkblue}{Adding data structures to Fortran in 1976 ({\it Initiation to Hydra} by R.K. B\"ock):}

\begin{center}
\includegraphics[width=0.8\linewidth]{img/hydra-3.png}
\end{center}
\end{frame}
...
\item<3-> In Python, we'd have to choose between fast NumPy, limited to rectangular structures, and slow-but-general Python.
\end{itemize}
...
\begin{center}
\uncover<4->{Awkward Array is a way to get variable-length lists and data structures {\it as well as} NumPy-like performance.}
\end{center}
\end{minipage}
\end{center}
\end{frame}
